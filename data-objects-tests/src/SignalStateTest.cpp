...
BOOST_AUTO_TEST_CASE( testLoadSetStateRequest ) {
  ...
  for(int i = 0; i < 5; ++i) {
    SignalId id(boost::lexical_cast<std::string>(i));
    AnalogState::Ptr as = boost::dynamic_pointer_cast<AnalogState>(request.getSignal(id));
    BOOST_CHECK_EQUAL(as->getValue(), (1.0 + 0.001 * i));
  }
  for(int i = 5; i < 10; ++i) {
    SignalId id(boost::lexical_cast<std::string>(i));
    BooleanState::Ptr bs = boost::dynamic_pointer_cast<BooleanState>(request.getSignal(id));
    BOOST_CHECK_EQUAL(bs->getValue(), true);
  }
}
...
BOOST_AUTO_TEST_CASE( testSaveSetStateResponse ) {
  uint8_t expected[] = {
    0x0a, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x30, 0x00, 
    0x01, 0x00, 0x00, 0x00, 0x31, 0x00, 0x01, 0x00, 0x00, 0x00, 
    0x32, 0x00, 0x01, 0x00, 0x00, 0x00, 0x33, 0x00, 0x01, 0x00, 
    0x00, 0x00, 0x34, 0x00, 0x01, 0x00, 0x00, 0x00, 0x35, 0x00, 
    0x01, 0x00, 0x00, 0x00, 0x36, 0x00, 0x01, 0x00, 0x00, 0x00, 
    0x37, 0x00, 0x01, 0x00, 0x00, 0x00, 0x38, 0x00, 0x01, 0x00, 
    0x00, 0x00, 0x39, 0x00
  };
...
BOOST_AUTO_TEST_CASE( testLoadSetStateResponse ) {
  ...
  for(int i = 0; i < 10; ++i) {
    SignalId id(boost::lexical_cast<std::string>(i));
    BOOST_CHECK_EQUAL(response.getResult(id), 0);
  }
}
